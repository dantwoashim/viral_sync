        ...
        .map(|acc| match acc {
            AccountField::Field(acc) => {
                ...
                let acc_type_path = match &acc.ty {
                    ...
                    // Interface accounts still belong in the instruction account list,
                    // but many non-Anchor wrapper types such as token-interface Mint and
                    // TokenAccount do not implement the IDL type/discriminator helpers that
                    // Anchor expects here. Skipping the type metadata keeps IDL generation
                    // compatible without changing runtime account validation.
                    Ty::InterfaceAccount(_) => None,
                    _ => None,
                };
