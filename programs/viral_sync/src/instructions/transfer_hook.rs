...
pub fn execute_transfer_hook(ctx: Context<ExecuteHook>, amount: u64) -> Result<()> {
    ...
    if is_from_treasury {
        ...
        if write_result.is_err() {
            ...
            dst_gen.dead_balance = dst_gen.dead_balance.checked_add(amount).ok_or(ViralSyncError::MathOverflow)?;
        } else {
            dst_gen.gen1_balance = dst_gen.gen1_balance.checked_add(amount).ok_or(ViralSyncError::MathOverflow)?;
        }
        return Ok(());
    }
    ...
    if is_to_vault && !is_from_merchant {
        ...
        let total_gen2_before = src_gen.gen2_balance.checked_add(gen2_consumed).ok_or(ViralSyncError::MathOverflow)?;
        for i in 0..src_gen.active_referrer_slots as usize {
            if src_gen.referrer_slots[i].is_active && total_gen2_before > 0 {
                src_gen.redemption_slot_consumed[i] = gen2_consumed
                    .checked_mul(src_gen.referrer_slots[i].tokens_attributed).ok_or(ViralSyncError::MathOverflow)?
    ...
    if is_src_intermediary {
        ...
        if !is_dst_intermediary {
            ...
            if write_inbound(dst_gen, entry).is_ok() {
                match entry_type {
                    GenSource::ViralShare => dst_gen.gen2_balance = dst_gen.gen2_balance.checked_add(amount).ok_or(ViralSyncError::MathOverflow)?,
                    _ => dst_gen.dead_balance = dst_gen.dead_balance.checked_add(amount).ok_or(ViralSyncError::MathOverflow)?,
                }
            } else {
                dst_gen.dead_balance = dst_gen.dead_balance.checked_add(amount).ok_or(ViralSyncError::MathOverflow)?;
            }
    ...
    require!(from_dead <= src_gen.dead_balance, ViralSyncError::InsufficientBalance);
    src_gen.gen1_balance = src_gen.gen1_balance.checked_sub(from_gen1).ok_or(ViralSyncError::MathOverflow)?;
    src_gen.gen2_balance = src_gen.gen2_balance.checked_sub(from_gen2).ok_or(ViralSyncError::MathOverflow)?;
    src_gen.dead_balance = src_gen.dead_balance.checked_sub(from_dead).ok_or(ViralSyncError::MathOverflow)?;
...
fn is_registered_vault(vault_account: &UncheckedAccount) -> bool {
    ...
    let Ok(data) = vault_account.try_borrow_data() else {
        return false;
    };
    let mut account_data: &[u8] = &data;
    let Ok(entry) = VaultEntry::try_deserialize(&mut account_data) else {
        return false;
    };
...
fn fifo_deduct(gen: &mut TokenGeneration, amount: u64) -> Result<()> {
    ...
    let from_dead = amount - from_gen1 - from_gen2;
    require!(from_dead <= gen.dead_balance, ViralSyncError::InsufficientBalance);
    gen.gen1_balance = gen.gen1_balance.checked_sub(from_gen1).ok_or(ViralSyncError::MathOverflow)?;
    gen.gen2_balance = gen.gen2_balance.checked_sub(from_gen2).ok_or(ViralSyncError::MathOverflow)?;
    gen.dead_balance = gen.dead_balance.checked_sub(from_dead).ok_or(ViralSyncError::MathOverflow)?;
    Ok(())
}
