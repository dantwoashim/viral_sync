...
pub fn process_redemption_slot(ctx: Context<ProcessRedemptionSlot>, slot_idx: u8) -> Result<()> {
    ...
    require!(slot_idx < gen.active_referrer_slots && (slot_idx as usize) < gen.referrer_slots.len(), ViralSyncError::InvalidReferrerSlot);

    let slot = gen.referrer_slots[slot_idx as usize];
    require!(slot.is_active, ViralSyncError::InvalidReferrerSlot);
    require_keys_eq!(referral.referrer, slot.referrer, ViralSyncError::InvalidReferrerSlot);
    require_keys_eq!(referral.referred, gen.owner, ViralSyncError::InvalidReferrerSlot);
    require_keys_eq!(ledger.referrer, referral.referrer, ViralSyncError::InvalidReferrerSlot);
    require_keys_eq!(ledger.merchant, referral.merchant, ViralSyncError::InvalidReferrerSlot);
    require_keys_eq!(ledger.mint, referral.mint, ViralSyncError::InvalidReferrerSlot);
    if slot.referral_record != Pubkey::default() {
        require_keys_eq!(referral_key, slot.referral_record, ViralSyncError::InvalidReferrerSlot);
    }
    ...
    if gen2_consumed > 0 && referral.is_active {
        ...
        let commission_exact_u128 = (gen2_consumed as u128)
            .checked_mul(referral.committed_commission_bps as u128).ok_or(ViralSyncError::MathOverflow)?;
