...
const VIRAL_SYNC_PROGRAM_ID = 'AeKT1B58Qi9rBtrtnMe11o4eXbVwHweKxGFNS5X3Vv46';
...
export function getFeeModelFinalization() {
  return {
    ...
    saasTiers: [
      { tier: 'Pilot', recurringNpr: 0, includedVisits: 25 },
      { tier: 'Local', recurringNpr: 2_500, includedVisits: 150 },
      { tier: 'Growth', recurringNpr: 7_500, includedVisits: 600 },
    ],
    interviewPrompts: ['Would verified-visit pricing feel fair?', 'Which cap makes finance approval easy?', 'Would a recurring tier reduce invoice friction?'],
  };
}
...
export async function getRevenueDashboard() {
  ...
  return {
    mrrNpr: getFeeModelFinalization().saasTiers[1].recurringNpr,
...
export async function getPaidMerchantPush() {
  ...
  return {
    ...
    objectionLog: [
      ...
      { objection: 'Need budget cap.', response: 'Use verified visit cap and recurring tier.' },
...
export function getRecurringCampaignTemplates() {
  return [
    ...
    { cadence: 'recurring', template: 'neighborhood route pass', reminder: 'first-week planning reminder' },
  ];
}
