...
const gates: Gate[] = [
  ...
  { id: 'fraud-gauntlet', label: 'Fraud Gauntlet', status: gauntletReady ? 'PASS' : gauntlet ? 'PENDING' : 'FAIL', detail: gauntletReady ? `${gauntlet?.summary?.blocked ?? 0}/${gauntlet?.summary?.totalCases ?? 0} attacks blocked with expected error evidence.` : `${gauntlet?.summary?.blocked ?? 0}/${gauntlet?.summary?.totalCases ?? 0} cases currently blocked.` },
