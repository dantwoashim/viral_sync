...
function allPassportFacts(passport: any) { const facts = passport?.verifiedFacts ?? {}; return ['campaignFunded','receiptRecorded','rewardSettled','nullifierRecorded','intentManifestCommitted','verifierOk','terminalVerified','visitorVerified','lineageVerified','settlementVerified'].every((key) => facts[key] === true); }
...
const gauntletReady = (gauntlet?.summary?.totalCases ?? 0) >= 15 && gauntlet?.summary?.blocked === gauntlet?.summary?.totalCases && gauntlet?.summary?.missing === 0 && gauntlet?.summary?.failed === 0 && (gauntlet?.cases ?? []).every((item: any) => item.observed === 'rejected' && item.expectedErrorMatched === true && item.accountsMutated === false && item.accountsMutationVerified === true);
...
const gates: Gate[] = [
  ...
  { id: 'fraud-gauntlet', label: 'Fraud Gauntlet', status: gauntletReady ? 'PASS' : gauntlet ? 'PENDING' : 'FAIL', detail: gauntletReady ? '15/15 attacks blocked with expected error evidence.' : `${gauntlet?.summary?.blocked ?? 0}/${gauntlet?.summary?.totalCases ?? 0} cases currently blocked.` },
