...
function sanitize(chunk) {
  return chunk
    ...
    .replace(/Recover the intermediate account's ephemeral keypair file[\s\S]*?Or to recover the account's lamports[\s\S]*?={20,}/gi, '[REDACTED_SOLANA_DEPLOY_RECOVERY_BLOCK]\n')
    .replace(/\([A-Z]:\\[^)\n\r]+\)/g, '([REDACTED_LOCAL_PATH])')
    .replace(/C:\\Users\\[^"\n\r]+\\\.config\\solana\\id\.json/gi, '[REDACTED_SOLANA_WALLET]')
    .replace(/C:\\Users\\[^"\n\r]+/gi, '[REDACTED_USER_HOME]')
