...
const CASE_DEFS: Array<Omit<GauntletCase, 'observed' | 'status' | 'errorCode' | 'expectedErrorCode' | 'actualError' | 'expectedErrorMatched' | 'accountsMutated' | 'accountsMutationVerified' | 'proofSource' | 'evidence' | 'failureKind'> & { fallbackError: string }> = [
  ...
  { id: 'child-parent-receipt-hash-mismatch', title: 'Child claim pass with forged parent receipt hash', expected: 'rejected', instruction: 'record_causal_receipt', normalReferralFailure: 'A referral tree can point at one parent receipt while committing another parent hash.', viralSyncDefense: 'Child claim passes must supply the actual settled parent receipt account and matching parent receipt hash.', fallbackError: 'InvalidLineageProof' },
